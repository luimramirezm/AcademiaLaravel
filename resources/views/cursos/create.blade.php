...
<form action="/cursos" method="POST" enctype='multipart/form-data'>
    {{-- en action debemos invocar la ruta hacia donde abre cuando le damos click al botonel metodo post siempre va para los formulario y el enctype se usa para enviar archivos como fotos --}}
    @csrf{{--Este es el token de botstrapt- la proteccion contra taques --}}
    @if ($errors->any()) {{-- errors es el atributo de laravel para le manejo en caso de errores --}}
        @foreach ($errors ->all() as $alerta) {{-- usamos el foreach y asi por cada error mandara un mensaje de alerta --}}
            <div class="alert alert-danger" role="alert"> {{-- Esto nos permite darle estilo a los mensajes de alerta con ayuda de bootstrapt--}}
                <ul>
                    <li>{{$alerta}}</li>
                </ul>
            </div>
